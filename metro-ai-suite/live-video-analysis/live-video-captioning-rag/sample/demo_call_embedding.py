...
"""
Standalone demo client for the /api/embedding endpoint.

What this script does:
1. Downloads a sample image from the edge-ai-libraries repository.
2. Converts the image bytes into a base64 string.
3. Sends the payload to the embedding endpoint so the backend can store
...
DEFAULT_EMBEDDING_URL = "http://localhost:4172/api/embedding"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Demo client for /api/embedding that stores an embedding in VDMS."
    )
    parser.add_argument(
        "--embedding-url",
        default=DEFAULT_EMBEDDING_URL,
        help=f"Embedding endpoint URL (default: {DEFAULT_EMBEDDING_URL})",
    )
    parser.add_argument(
        "--image-url",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Posting embedding request to: {args.embedding_url}")
